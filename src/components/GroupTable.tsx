...
const GroupTable: React.FC<GroupTableProps> = ({ group, teams, qualifyCount = 2 }) => {
          ...
          <tbody>
            {group.standings.map((standing, index) => {
              ...
              return (
                <tr 
                  key={standing.teamId}
                  className={`table-row-striped border-b border-border last:border-0 transition-colors ${
                    isQualified ? 'bg-court-green-light/50' : ''
                  }`}
                >
                  ...
                  <td className="px-4 py-3">
                    <span className={`font-medium ${isQualified ? 'text-primary' : 'text-foreground'}`}>
